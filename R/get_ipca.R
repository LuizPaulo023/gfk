...
get_ipca <- function(){
  ...
  db_ipca <- ipca_raw %>%
             ...
             dplyr::select(mes,
                           variavel,
                           geral_grupo_subgrupo_item_e_subitem,
                           valor) %>%
             tidyr::pivot_wider(names_from = variavel,
                                values_from = valor) %>%
              # Expressão regular
             dplyr::mutate(items = gsub('^[[:digit:]]+', "",
                                        geral_grupo_subgrupo_item_e_subitem),
  ...
  ifelse(is.na(db_ipca$product_groups),
         print("Algum item não está especificado no grupo de produtos"),
         print("Todos os items estão especificados no grupo de produtos"))
